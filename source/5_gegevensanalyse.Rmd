...
```{r figmodel, warning=FALSE, fig.cap= "Visualisatie analysemodel voor Gevlekte witsnuitlibel"}
...
ggplot(data = doy_year_loc_effect_2, aes(x = doy)) + 
...
* de maximale telling binnen het telseizoen per jaar.
...
## Interpretatie van trends/verschillen {#h:classtrend}
...
```{r figtrend, fig.cap= "Classificatie van verschillen/trends", cache = FALSE}
...
ggplot(trend_klasses, aes(x = klasse_naam, y = trend, ymin = lcl, ymax = ucl)) +
  ...
  labs(x= "beschrijving klasse", y = "verschil/trend") +
  coord_flip()
...
if(doc.type == "docx"){
 ...
 trend_klasses %>%
  ...
  kable(format = "pandoc",
        align = c("c", "l", "l"),
        caption = "Classificatie van verschillen/trends",
        booktabs = TRUE
        )
  ...
  select("Code" = klasse_tabel, "Klasse" = klasse_naam, "Beschrijving" = klasse_beschrijving) %>%
  kable(align = c("c", "l", "l"),
        caption = "Classificatie van trends",
        booktabs = TRUE) %>%
    kable_styling()
...
Ook andere jaren kunnen als referentiejaar gebruikt worden. Om alle combinaties te visualiseren stellen we \@ref(fig:indexcomb) voor met in de y-as het referentiejaar en in de x-as het jaar dat vergeleken wordt met het referentiejaar. De symbolen tonen het type verschillen tussen de jaren volgens het classificatiesysteem in Figuur  \@ref(fig:figtrend) en Tabel \@ref(tab:tabelklasse). De kleur geeft de richting en de grootte van de verandering aan. We zien bijvoorbeeld voor de Gevlekte witsnuitlibel in 2018 een sterke toename t.o.v. 2017. Dit wordt weergegeven via een blauw gearceerde driehoek. Als we 2017 vergelijken t.o.v. 2018 als referentiejaar, dan krijgen we uiteraard hetzelfde resultaat maar dan in de andere richting.     
...
overzicht_compare_years %>%
  ...
  coord_fixed() +
  scale_colour_gradient2("verandering", breaks = breaks, labels = sprintf("%+.0f%%", labels), low = "red", high = "blue") +
  scale_effect() +
  scale_y_continuous(breaks = c(2016, 2017, 2018)) +
  scale_x_continuous(breaks = c(2016, 2017, 2018)) +
  labs(y = "Referentiejaar", x = "Jaar") +
...
Figuur \@ref(fig:figtrend-resultaat) toont de gemiddelde jaarlijkse trend over de periode 2016-2018. Om het type trend te bepalen gebruiken we hier een jaarlijkse trend van  `r treshold_low`% als onderste  drempelwaarde en van `r treshold_high` % als bovenste drempelwaarde. Over de volledige trendperiode (3 jaar) komt dit overeen met een respectievelijk een afname van -25% en een toename van +33%
...
In Tabel \@ref(tab:tabtrendresultaat) geven we een overzicht van de jaarlijkse trends en duiden we ook aan of de trend al dan niet lineair is. 
