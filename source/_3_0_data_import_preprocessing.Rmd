...
overzicht_taken_uitgevoerd_wide <- overzicht_taken_uitgevoerd %>%
  ...
  select(soortgroep, meetnet, protocol, jaar, locatie, is_sample, is_active, taak, telling_ingevoerd, telling_cp, telling_gvm, telling_nvcp)

overzicht_locaties <- overzicht_taken_uitgevoerd_wide %>%
  group_by(soortgroep, meetnet, protocol, jaar, locatie, is_sample, is_active) %>%
  ...
  ungroup() %>%
  full_join(taken_nu, by = c("soortgroep", "meetnet", "protocol","locatie", "jaar")) %>%
...
evaluatie_locaties <- evaluatie_locaties %>%
  ...
  ungroup() %>%
 arrange(desc(is_sample), soortgroep, meetnet, protocol, locatie)
...
overzicht_meetnetten <- evaluatie_locaties %>%
  filter(jaar < 2019) %>%
  filter(soortgroep != "0") %>%
  group_by(is_sample, soortgroep, meetnet, protocol, jaar, Meetcyclus) %>%
  ...
  group_by(soortgroep, meetnet, protocol, Meetcyclus, is_sample) %>%
...
data_extra <- read.csv2("../Data/OverzichtSoortenBuitenMN_versie20190109.csv") %>%
  rename(is_sample = Steekproeflocatie, aantal_tellingen_cp = AantalTellingenCP,
...
overzicht_meetnetten_all <- overzicht_meetnetten %>%
  ...
  filter(!meetnet %in% (filter(data_extra, is_sample == 1 & jaar == 2018))$meetnet) %>%
...
overzicht_planten2 <- overzicht_planten %>%
  mutate(is_sample = ifelse(type == "inMeetnet", 1, 0),
...
tabel_stuurgroep <- overzicht_meetnetten_all %>%
  ...
  mutate(aantal_eenheid_doel = ifelse(Eenheid == "Locaties" & is_sample == 1, aantal_voldoende_geteld,
         ifelse(Eenheid == "Nieuwe locaties" & is_sample == 1, aantal_voldoende_geteld_nieuw,
                ifelse(Eenheid == "Bezoeken" & is_sample == 1, aantal_tellingen_cp, 
                       ifelse(Eenheid == "Hectare" & is_sample == 1, OppervlakteGeteld, NA)))),
    doelbereik_jaar = aantal_eenheid_doel/Jaardoel * 100,
         doelbereik_cyclus = ifelse(!is.na(Cyclusdoel) & is_sample == 1, aantal_voldoende_geteld_cyclus/Cyclusdoel * 100, NA),
...
voortgang_meetnetten_sg <- tabel_stuurgroep %>%
  ...
  group_by(is_sample, meetnet_type, soortgroep, jaar) %>%
  summarise(aantal_tellingen_cp = sum(aantal_tellingen_cp),
...
voortgang_meetnetten <- tabel_stuurgroep %>%
  group_by(is_sample, meetnet_type, soortgroep, jaar, meetnet, protocol) %>%
  summarise(aantal_tellingen_cp = sum(aantal_tellingen_cp),
            aantal_voldoende_geteld = sum(aantal_voldoende_geteld),
            totaal_doel = sum(Jaardoel),
